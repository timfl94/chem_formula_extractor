...
def get_auth_token(username: str=HYPLAG_USER, password: str=HYPLAG_PASSWORD) -> Tuple[str, str]:
    token_time = str(datetime.utcnow())
    headers: Dict[str, str] = {"Content-Type": "application/json",
                               "Accept": "application/json"}
    payload: str = f"{{\"name\": \"{username}\", \
                       \"password\": \"{password}\"}}"
...
def post_document(path_to_document: Path, token: str, external_id: str=HYPLAG_ID) -> int:
    ...
    files = {"external_id": (None, f"{external_id}"),
...
def post_documents_from_list(list_of_documents: List[Path], token: str, external_id: str=HYPLAG_ID, num_threads: int=10) -> List[int]:
    document_id_list = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(num_threads, len(list_of_documents))) as executor:
        doc_ids = [executor.submit(post_document, str(path), token, external_id) for path in list_of_documents]
        for doc_id in concurrent.futures.as_completed(doc_ids):
           document_id_list.append(doc_id.result())
    return document_id_list

def post_documents_from_folder(folder: Path, token: str, external_id: str=HYPLAG_ID) -> List[int]:
...
def get_document(document_id: int, token: str):
    get_url = HYPLAG_BACKEND_GET_DOCUMENT + str(document_id)
...
def get_documents_from_list(list_document_ids: List[int], token: str, num_threads: int):
    ...
    xml_document_list = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(num_threads, len(list_document_ids))) as executor:
            paper_xml_download = [executor.submit(get_document, doc_id, token) for doc_id in list_document_ids]
...
def process_documents_grobid(path_to_pdf: Path = Path(PDF_FILES), num_threads: int = 10):
    ...
    client.process("processFulltextDocument", input_path=path_to_pdf, output=XML_FILES, n=num_threads)

if __name__ == "__main__":
    token = get_current_token()
    doc_id = post_document(Path(PDF_FILES + "acssuschemeng.7b03870.pdf"), token)
    doc = get_document(doc_id, token)
    save_xml_doc(path_to_save=Path("test.xml"), xml_doc=doc)
    print("test")
